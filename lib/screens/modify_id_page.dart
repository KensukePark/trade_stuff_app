...
class modifyPwPage extends StatefulWidget {
  const modifyPwPage({Key? key, required this.cur_id}) : super(key: key);
  final cur_id;
  @override
    ...
    if (user != null) {
      user.updatePassword(password).then((_) {
        ScaffoldMessenger.of(context)
          ..hideCurrentSnackBar()
          ..showSnackBar(SnackBar(content: Text('비밀번호 수정 완료')));
        Navigator.pop(context);
      }).catchError((error) {
        ScaffoldMessenger.of(context)
          ..hideCurrentSnackBar()
          ..showSnackBar(SnackBar(content: Text('비밀번호 수정 실패')));
        Navigator.pop(context);
      });
    }
  }
